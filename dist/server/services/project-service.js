...
exports.default = ({ strapi }) => ({
    ...
    createAll: async (repos, userId) => {
        return Promise.all(repos
            .map(async (repo) => await strapi.plugin('github-projects').service('projectService').create(repo, userId)));
    },
    deleteAll: async (repositoryIds) => {
        return Promise.all(repositoryIds
            .map(async (repositoryId) => await strapi.plugin('github-projects').service('projectService').delete(repositoryId)));
    },
    find: async (params) => {
